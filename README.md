...
- **工作区目录选择**：侧栏底部「添加工作区」一键弹出 Windows 原生文件夹选择窗口（WSL 下经 interop 启动 `powershell.exe`，选中的 Windows 路径自动经 `wslpath` 转换为 WSL 路径），并提供已注册工作区快捷列表与手动输入（支持 `~`、Windows 路径、相对路径）。选中后沿用 DSH 官方 New Session 流程注册并打开工作区；无图形界面时自动降级为快捷列表 + 手动输入。
