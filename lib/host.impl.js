...
function getPanelState(root, sessionId) {
  ...
  // 会话级面板状态不能继承 workspace/root 的旧会话状态：否则新会话
  // 第一次解析出 cwd 后，会把旧会话的 open/collapsed 状态误带进来。
  // 仅无 sessionId 的兼容调用才使用 root 级状态。
  return doc.byRoot[key] ?? (sessionId ? null : (doc.byRoot[root] ?? null))
}
...
//#region src/host/folder-picker.js
...
//#endregion src/host/folder-picker.js
    ...
    if (rest === '/prompt-optimize' && method === 'POST') {
      ...
      const resolved = typeof services.llm.resolveCallConfig === 'function'
        ...
        : { provider, model, ...(reasoningEffort ? { reasoningEffort } : {}) }
            // 提示词增强（Prompt Enhancer）：轻量改写调用，把模糊需求补全为结构化指令，
      // 只改写文本，不触发任务执行；结果仅回填输入框，不进入会话历史。
      const system = '你是一名提示词优化专家（Prompt Enhancer）。用户会给你一句（或几句）模糊的任务描述，请将其改写为一条结构化、可执行的完整指令。\n\n要求：\n1. 保留用户的原始意图，不擅自添加新需求；\n2. 补全缺失的要素：目标（要做什么）、角色/背景（如有）、输入（用到的文件/数据）、输出格式（文档/表格/代码/列表等）、约束（字数/语气/禁止事项）；\n3. 用词具体、可执行，避免「帮我弄一下」「尽量好一点」这类模糊表达；\n4. 仅输出优化后的指令本身，不要解释、不要加前缀、不要使用代码围栏。'
      const messages = [{
